...
@Component({
  selector: 'app-soft-pin',
  templateUrl: './soft-pin.component.html',
  styleUrls: ['./soft-pin.component.scss']
})

export class SoftPinComponent implements OnInit {
  ...
  constructor(public rest: RestService, private spinner: NgxSpinnerService, public alerts: NotificationService,) { }
  ...
  retrieveData() {
    ...
    this.rest.postValidate(postData, 'soft/softpinlist').subscribe(
      (res: any) => {
        if (res.error) {
          this.spinner.hide();
          this.showCardDetails = false;
          this.alerts.errorAlert(`Soft Pin`, res.respDesc);
          return;
        }
  ...
  submitstatus() {
    ...
    this.rest.postValidate(reqData, 'soft/otprequest').subscribe((res: any) => {
      if (res.respCode == '00') {
        this.spinner.hide();
        this.alerts.successAlert(`Soft Pin`, res.respstatus);
      } else {
        this.spinner.hide();
        this.alerts.errorAlert(`Soft Pin`, res.respstatus);
      }
    })
