...
export class CustomerInfoComponent {
  ...
  retrieveData() {
    ...
    const url = 'cardmaintain/customerinfo';
    ...
    this.rest.postValidate(postData, url).subscribe(
      (res: any) => {
        ...
        if (res.respCode === '00') {
          this.cardDataList = res.customerDetailsList;
          this.chnMaskList = this.cardDataList.map(item => item.chnMask);

          if (this.chnMaskList.length > 0) {
            this.selectedChnMask = this.chnMaskList[0];
            this.onChnMaskChange(this.selectedChnMask);
          }

          this.showCardDetails = true;
        } else {
          this.showCardDetails = false;
          this.alerts.errorAlert('Customer Details', res.respDesc);
        }
      },
      (error) => {
        this.spinner.hide();
  ...
  onChnMaskChange(selectedMask: string) {
    ...
    const selectedCardData = this.cardDataList.find(item => item.chnMask === selectedMask);
    if (!selectedCardData) return;

    this.populateFormData(selectedCardData);
  }

  populateFormData(cardData: any) {
    this.isEditMode = true;

    this.firstFormGroup.patchValue({
      productcode: cardData.prodCode || '',
    ...
    this.thirdFormGroup.patchValue({
      ...
      encryptChn: cardData.encryptChn || '',
    ...
    if (cardData.accttype) {
      this.getAccSubTypeList(cardData.accttype);
    }
    if (cardData.subAcctType && cardData.prodCode) {
      this.getFee(cardData.subAcctType, cardData.prodCode);
    }
  }
  ...
  addcustomerEntry() {
    ...
    this.rest.postValidate(postData, url).subscribe(
      (res: any) => {
        if (res.respCode === '00') {
          this.spinner.hide();
          this.alerts.successAlert("Customer Details", res.respDesc);
