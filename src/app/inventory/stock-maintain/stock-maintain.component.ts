...
@Component({
  selector: 'app-stock-maintain',
  templateUrl: './stock-maintain.component.html',
  styleUrls: ['./stock-maintain.component.scss'],
  providers: [DatePipe]
})


export class StockMaintainComponent implements OnInit {
  ...
  //displayedColumns: string[] = ['prodCode', 'PBranchCode','easyLifeCnt','easyLifeFlexCnt','easyLifePlusCnt'];
  displayedColumns: string[] = ['prodCode', 'PBranchCode','count'];
  ...
  getFilterRecordList() {
    ...
    this.rest.postValidate(reqData, url).subscribe((res: any) => {
      if (res.respCode == '00') {
        ...
        this.spinner.hide();
        this.cdr.detectChanges();
        this.alerts.errorAlert('Stock Maintain', res.respDesc);
      }
    })
  }
  ...
  generatecvv() {
    if (this.selection.selected.length == 0) {
      this.alerts.showAlert('Stock Maintain', 'Please select the records');
    }
    else {
      this.spinner.show();
      let reqcvvgenerate = {
      ...
      this.rest.postValidate(reqcvvgenerate, url).subscribe((res: any) => {
        ...
        if (res.respCode == '00') {
          ...
          this.selection.clear();
          this.cdr.detectChanges();
          this.alerts.successAlert('Stock Maintain', res.respDesc);
