...
export class HsmComponent implements OnInit {
  ...
  InitForm() {
    this.hsmform = this.fb.group({
      hsmName: ['', [Validators.required, Validators.maxLength(24), Validators.minLength(6), Validators.pattern("^[a-zA-Z ]*$")]],
      decimilisation: ['', Validators.pattern("^[0-9]*$"),Validators.maxLength(16),Validators.minLength(16)],
      productCode: ['', Validators.required],
      messageHeader: ['', [Validators.required, Validators.maxLength(4),Validators.minLength(4),Validators.pattern("^[A-Z]*$")]],
      pinlength: ['', Validators.pattern("^[0-9]*$")],
      pinOffsetLength: ['', Validators.pattern("^[0-9]*$")],
      //pinMailerDesc: ['', [Validators.required, Validators.maxLength(25), Validators.pattern("^[a-zA-Z ]*$")]],
      // mailHeight: ['', Validators.pattern("^[0-9]*$")],
      ip: ['', [Validators.required, Validators.pattern("^[0-9.]*$"),Validators.minLength(7),Validators.maxLength(15)]],
      panOffsetLength: ['', Validators.pattern("^[0-9]*$")],
      pvk1: ['', [Validators.pattern('^[a-zA-Z0-9]*$'), Validators.minLength(16), Validators.maxLength(16)]],
      cvk1: ['', [Validators.pattern('^[a-zA-Z0-9]*$'), Validators.minLength(16), Validators.maxLength(17)]],
      panPadChar: ['', Validators.pattern('^[a-zA-Z]*$')],
      // mailLength: ['', Validators.pattern("^[0-9]*$")],
      port: ['', [Validators.required, Validators.maxLength(4), Validators.minLength(4), Validators.pattern("^[0-9]*$")]],
      panValLength: ['', Validators.pattern("^[0-9]*$")],
      pvk2: ['', [Validators.pattern('^[a-zA-Z0-9]*$'), Validators.minLength(16), Validators.maxLength(16)]],
      cvk2: ['', [Validators.pattern('^[a-zA-Z0-9]*$'), Validators.minLength(16), Validators.maxLength(16)]]
    })
  }
  ...
  getHSMList() {
    this.spinner.show();
    this.rest.getwithHeader('hsm/get').subscribe(
      res => {
        if (res.respCode == "00") {
          this.spinner.hide();
  ...
  onRowClick(item: any, action: any) {
    this.mode = action;
    if (this.mode === "E") {
      this.hsmform.controls['productCode'].disable();
    } else {
      this.hsmform.controls['productCode'].enable();
    }
  ...
  deletehsm(item: any) {
    Swal.fire({
      title: 'Are you sure?',
      text: "Do You want to delete-" + item.productCode + "!",
      icon: 'warning',
  ...
  initDeleteApi(item) {
    ...
    var url = "hsm/delete" + this.productCode;

    this.rest.postValidate(deleteData, "hsm/delete").subscribe((res: any) => {

      if (res.respCode == "00") {
        this.spinner.hide();
        this.alertService.successAlert('HSM Details', res.respDesc);
        setTimeout(() => {
          this.backList();
          this.getHSMList();
        }, 3000);
        ...
        this.spinner.hide();
        this.alertService.errorAlert('HSM Details', res.respDesc);
      }
    });
  }
