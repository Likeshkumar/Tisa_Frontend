...
@Component({
  selector: 'app-cvv-generation-pers',
  templateUrl: './cvv-generation-pers.component.html',
  styleUrls: ['./cvv-generation-pers.component.scss'],
  providers: [DatePipe]
})
export class CvvGenerationPersComponent implements OnInit {
  ...
  getFilterRecordList() {
    ...
    let url = 'personalized/auth/get/cvvlist';
  ...
  generatecvv() {
    if (this.selection.selected.length == 0) {
      this.alerts.showAlert('Please select the records', 'Generate CVV');
    }
    else {
      this.spinner.show();
      let reqcvvgenerate = {
