...
@Component({
  selector: 'app-issused-report',
  templateUrl: './issused-report.component.html',
  styleUrls: ['./issused-report.component.scss'],
  providers: [DatePipe]
})

export class IssusedReportComponent implements OnInit {
  productlist: any;
  branchlist: any;
  ...
  encrypt(format: string) {
    ...
    let url = 'report/productionReport';
    ...
    this.rest.postValidate3(postData, url, { responseType: 'blob' }).subscribe((res: Blob) => {
      ...
      const fileName = `IssusedCardReport_${currentDateTime}.${format}`;
