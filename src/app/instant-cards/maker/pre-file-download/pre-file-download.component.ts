...
@Component({
  selector: 'app-pre-file-download',
  templateUrl: './pre-file-download.component.html',
  styleUrls: ['./pre-file-download.component.scss']
})

export class PreFileDownloadComponent implements OnInit {
  ...
  getbatchlist() {
    ...
    this.rest.postValidate(reqData, url).subscribe((res: any) => {
      if (res.respCode == '00') {
        ...
        this.spinner.hide();
        this.alerts.showError('Batch List Details', res.respDesc,);
  ...
  downloadPre() {
    ...
    let url = 'instant/encoding/pre/download';
