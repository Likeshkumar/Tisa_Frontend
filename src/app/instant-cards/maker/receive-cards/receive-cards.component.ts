...
@Component({
  selector: 'app-receive-cards',
  templateUrl: './receive-cards.component.html',
  styleUrls: ['./receive-cards.component.scss']
})


export class ReceiveCardsComponent implements OnInit {
  ...
  getbatchlist() {
    ...
    let url = 'get/batchInsReceiveList';
  ...
  getFilterRecordList() {
    if (this.batchnum == null || this.batchnum == undefined) {
      ...
      let url = 'instant/receive/card-list';
      this.rest.postValidate(reqData, url).subscribe((res: any) => {
        if (res.respCode == '00') {
          this.spinner.hide();
  ...
  receivecard() {
    if (this.selection.selected.length == 0) {
      ...
      let url = 'instant/receive/card-generate';
