...
@Component({
  selector: 'app-auth-branch-issuance',
  templateUrl: './auth-branch-issuance.component.html',
  styleUrls: ['./auth-branch-issuance.component.scss'],
  providers: [DatePipe]
})


export class AuthBranchIssuanceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.issueinstantprocode = this.rest.instantissuecardprocesscodeAuth;
    this.getProductData();
    this.getBranchData();
  }
  ...
  getFilterRecordList() {
    ...
    let url = 'instant/issuance/authCard-list';
  ...
  authorizeIssueCard() {
    if (this.selection.selected.length == 0) {
      this.alerts.showAlert('Please select records', 'Branch Issuance of Cards');
    } else {
      this.spinner.show();
      ...
      let url = 'instant/issuance/authorize';
  ...
  rejectIssueCard() {
    if (this.selection.selected.length == 0) {
      ...
      let url = 'instant/issuance/rejectAuthorize';
      this.rest.postValidate(reqDataauthorise, url).subscribe((res: any) => {
