...
@Component({
  selector: 'app-auth-card-order',
  templateUrl: './auth-card-order.component.html',
  styleUrls: ['./auth-card-order.component.scss'],
  providers: [DatePipe],

})

export class AuthCardOrderComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'productcode', 'branchcode', 'cardTypeCode', 'cardNo', 'expdate', 'customername'];
  ...
  getFilterRecordList() {
    ...
    let url = 'instant/card/getAuthList';
    this.rest.postValidate(this.reqData, url).subscribe((res: any) => {
      if (res.respCode == '00') {
        this.spinner.hide();
        this.cdr.detectChanges();
        this.data = res.AuthCardDetails;
        setTimeout(() => {
        ...
        this.spinner.hide();
        this.cdr.detectChanges();
        this.alerts.errorAlert('Instant  Card Order', res.respDesc);
      }
    })
  }
  ...
  rejectCardOrder() {
    if (this.selection.selected.length == 0) {
      this.alerts.showAlert('Please select the records', 'Instant  Card Order');
    }
    else {
      this.spinner.show();
      let reqcvvgenerate = {
      ...
      let url = 'instant/card/rejectAuthorize';
  ...
  authorizeCardOrder() {
    if (this.selection.selected.length == 0) {
      this.alerts.showAlert('Instant  Card Order', 'Please select the records');
    }
    else {
      this.spinner.show();
      let reqcvvgenerate = {
      ...
      let url = 'instant/card/authorize';
      this.rest.postValidate(reqcvvgenerate, url).subscribe((res: any) => {
